...
Additionally an adjustment for BCA subtype is included for the model of all participants (as opposed to the model stratified by BCA substype).
...
I believe age at diagnosis, race and ethnicity are all confounders (affect both exposure and outcome) and sample type is an instrumental variable (affects only the exposure, not related to anything else).  This would mean that adjusting for it would likely have a detrimental effect on our estimates.
...
- Gene features must have a >1% of cases positive (previously 0.5%).
- Plots changed to show log cumulative hazard ratio estimates (previously no log).
- Added comparison with models that do not have confounder adjustments, in other words genes only.

## Model changes

Proposed: Group all alterations for a gene together except in cases where a differential effect is known.  Two cases where a differential effect is known:

- ERBB2 mutations and CNAs will be split due to a known difference.
- GATA3 will be split into loss of function and gain of function mutations.
  - Brooke overviewed the oncogenic alterations and this is a case where both loss and gain are present in the same gene.
