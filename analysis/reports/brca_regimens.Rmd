...
```{r, plot-sun_class_one_point_one, include = T}
...
*Example interpretation using row 1 (may become incorrect with data updates):* Microtubule inhibitors were used by more participants in an index cancer regimen after distant metastatses (560) than any other drug.  Regimens containing microtubule inhibitors also frequently contained antiher2 drugs (42.7% of regimens with microtubule inhibitors contained antiher2 drugs) and platinum (5.4%).
...
```{r}
dft_drug_c1 <- prog_cohort_c1$cohort_ca_drugs %>% 
  # Get only the index cancers:
  left_join(
    select(
      prog_cohort_c1.1$cohort_ca_dx,
...
dft_drug_c1 <-
  left_join(dft_drug_c1, dft_drug_c1_char, 
            by = c("record_id", "regimen_number", "drug_id")) %>%
  relocate(drug, .before = drugs_startdt_int)
...
```{r}
dft_bca <- prog_cohort_c1$cohort_ca_dx %>%
...
This could be repeated for the finer classes of drug classification (e.g. Class 1.1, Class 1.2) if needed.
