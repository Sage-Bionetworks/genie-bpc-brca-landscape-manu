...
count(dft_ca_ind, record_id) %>% nrow # number of people in this cohort.
get_dmet_timing(dft_ca_ind) %>% nrow # number of people with met dx anytime
dft_her2_low %>%
  group_by(record_id) %>%
