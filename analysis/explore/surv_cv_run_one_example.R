...
cvfit_dmet_os <- cv.glmnet(
  ...
  y = y_dmet_os,
  standardize = T,
  alpha = 0.98, # small bit of ridge for better convergence.
  family = "cox",
  nfolds = 10
)

dft_coef_dmet_os <- get_cv_lasso_coefs(cvfit_dmet_os) 
plot(cvfit_dmet_os)

dft_coef_dmet_os_plot <- coef(cvfit_dmet_os, s = "lambda.min") %>% 
  tidy_cv_glmnet(., exp_coef = T, remove_zero = F)

gg_os_dmet_coef <- plot_coef_grid(
  dat = dft_coef_dmet_os_plot, 
